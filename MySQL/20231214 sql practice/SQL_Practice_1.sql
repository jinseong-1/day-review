...
select sum(sal * 6) as 급여합 from emp;

-- 10. 사원들의 6개월 커미션의 합을 구하라
select sum(comm * 6) as 커미션합 from emp;
...
select deptno, ename from emp where sal>=2000;
...
select deptno, ename from emp where job='clerk' and hiredate>19811231;
...
select deptno, ename from emp where sal BETWEEN 1000 and 2500;
