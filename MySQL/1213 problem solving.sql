...
select * from emp where sal and comm;
...
select hiredate from emp group by hiredate;		-- group by 도 중복 제거 가능
...
select sal, ename, deptno from emp where sal >= 2000;
...
select deptno,ename,mgr from emp where mgr!='null';
select deptno,ename,mgr from emp where mgr is not null;
-- 32. 매니저 번호가 7698 또는 7839인 사원의 사원번호, 이름을 검색하라

select detpno, ename from emp where mgr = 7698 or mgr = 7839;
select detpno, ename from emp where mgr in (7698, 7839);
...
-- SELECT ename from emp where deptno = 10; 10자리에 대입
